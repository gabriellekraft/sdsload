...
vignette: >
  %\VignetteIndexEntry{Using the loadsds package}
...
The main function of the sdsload package is to let you load and install all required packages for a number of statistical and data sciences classes at Smith College; this package streamlines the process and ensures all necessary packages are loaded at the beginning of your R session. Let's see how we can accomplish this. First, load the `load_sds` package.
...
The loadsds package comes with a built-in data set that stores available SDS courses and their corresponding packages:
